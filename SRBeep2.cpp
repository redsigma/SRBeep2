...
static std::u8string to_u8string(const std::string &value) { return std::u8string(value.begin(), value.end()); }
...
{
    if (!config.enable_input_capture_enforcement || config.input_capture_target.empty() ||
        config.input_capture_device_names.empty())
    {
        return;
    }
    ...
    if (!device_property)
    {
        blog(LOG_WARNING, "SRBeep2: Source '%s' does not expose a 'device_id' property.",
    ...
    if (matched_device_id.empty())
    {
        ...
        blog(LOG_INFO,
             ...
             config.input_capture_target.c_str(), requested_list.c_str(), available_list.c_str());
        obs_source_release(target_source);
        return;
    }
    ...
    if (matched_device_id != current_device_id)
    {
        ...
        blog(LOG_INFO, "SRBeep2: Source '%s' device set to '%s'.", config.input_capture_target.c_str(),
             matched_device_name.c_str());
    }
...
void play_clip(const char *filepath)
{
    ...
    while (!done_waiting_for_track)
    {
        bool track_stopped = false;
        {
            ...
            playbackStateCv.wait_for(
                lock, std::chrono::milliseconds(shutdown_wait_timeout_ms),
...
static void queue_deferred_sound_job(const char *file_name, bool apply_record_start_mute,
                                     SoundJob::DeferredAction deferred_action)
{
    {
        std::lock_guard<std::mutex> lock(audioJobMutex);
        SoundJob job;
...
static void audio_worker_loop()
{
    while (true)
    {
        SoundJob job;
        {
            ...
            audioJobCv.wait(lock, [] { return !audioWorkerRunning || !audioJobs.empty(); });
...
static void apply_hotkey_fallback_if_unbound(obs_hotkey_id hotkey_id, const std::string &fallback_spec,
                                             const char *hotkey_name)
{
...
extern "C" const char *obs_module_author(void) { return "EBK21"; }

extern "C" const char *obs_module_name(void) { return "Stream/Recording Start/Stop Beeps"; }
...
extern "C" bool obs_module_load(void)
{
    ...
    record_toggle_hotkey = obs_hotkey_register_frontend("srbeep2.toggle_recording_deferred",
                                                        "SRBeep2: Toggle Recording", on_record_toggle_hotkey, NULL);
    pause_toggle_hotkey = obs_hotkey_register_frontend("srbeep2.toggle_pause_deferred", "SRBeep2: Toggle Pause",
                                                       on_pause_toggle_hotkey, NULL);
